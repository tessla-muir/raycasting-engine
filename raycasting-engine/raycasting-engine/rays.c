...
void CastRay(float rayAngle, int stripId) {
	...
	while (nextHorizontalTouchX >= 0 && nextHorizontalTouchX <= MAP_COLS * TILE_SIZE && nextHorizontalTouchY >= 0 && nextHorizontalTouchY <= MAP_ROWS * TILE_SIZE) {
		...
		if (HasMapWallAt(xToCheck, yToCheck)) {
			...
			horizontalWallTex = GetMapAt((int)floorf(yToCheck / TILE_SIZE), (int)floorf(xToCheck / TILE_SIZE));
	...
	while (nextVerticalTouchX >= 0 && nextVerticalTouchX <= MAP_COLS * TILE_SIZE && nextVerticalTouchY >= 0 && nextVerticalTouchY <= MAP_ROWS * TILE_SIZE) {
		...
		if (HasMapWallAt(xToCheck, yToCheck)) {
			...
			VerticalWallTex = GetMapAt((int)floorf(yToCheck / TILE_SIZE), (int)floorf(xToCheck / TILE_SIZE));
	...
	if (verticalHitDist < horizontalHitDist) {
		...
		rays[stripId].texture = VerticalWallTex;
		...
		rays[stripId].texture = horizontalWallTex;
